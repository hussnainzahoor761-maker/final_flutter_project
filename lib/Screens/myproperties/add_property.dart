...
class _AddPropertyState extends State<AddProperty> {
  ...
  Future<void> _saveProperty() async {
    ...
    try {
      final response = await _apiServices.createProperty({
      ...
      if (propertyId != null && _selectedPhotos.isNotEmpty) {
        ...
        final mediaResponse = await _apiServices.uploadPropertyMedia(
          propertyId,
          mediaFiles,
        ...
        if (mediaResponse.statusCode == null ||
          ...
          debugPrint(
            'Property media upload failed: ${mediaResponse.statusCode}',
          );
        }
      }
      if (propertyId != null) {
        final publishResponse = await _apiServices.publishProperty(propertyId);
        if (publishResponse.statusCode == null ||
            publishResponse.statusCode! >= 300) {
          throw Exception(_apiErrorMessage(publishResponse.data));
        }
      }
      if (!mounted) return;
